                ...
                float output[])
{
    ...
    for (int i=0; i<N; i+=8) {
        ...
        char comparisonBits = _mm256_movemask_ps(comparisonVec);  // If all 0, then the while loop is finished
        while (comparisonBits != 0) {
            __m256 newGuess = _mm256_mul_ps(guess, guess);
            newGuess = _mm256_mul_ps(newGuess, guess);
            newGuess = _mm256_mul_ps(x, guess);
            newGuess = _mm256_sub_ps(_mm256_mul_ps(threeVec, guess), newGuess);
            newGuess = _mm256_mul_ps(newGuess, halfVec);
        }
        while (error > kThreshold) {
